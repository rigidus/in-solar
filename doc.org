...
1) есть неограниченное множество независимых исполнителей (VE)
2) вероятность, что два объекта будут исполняться одним исполнителем - ничтожна
3) объекты могут выполнять вызовы к друг-другу, один входящий вызов может породить любое число исходящих, но строго последовательно (в пределах входящего вызова)

задача:

предложить и обосновать механизм детектирования dead-lock'ов/рекурсии, который удовлетворяет следующим требованиям:

1) нулевой или константный overhead по cpu, памяти и трафику на вызовы, которые пересекаются на менее чем 2х общих объектах
2) затраты на определение коллизии на "листовом" исполнителе не зависят от глубины вызова
3) модель генерализуема до уровня транзакционного управления вызовами (



Введем Арбитра (A), который узнает о request в момент его появления.

Получив request_1, Арбитр передает его на исполнение узлу. В процессе
выполнения request_1 необходимо захватить объект Z. Перед захватом узла
объекта Z узел запрашивает разрешение у Арбитра и производит захват
только если разрешение получено. В этот момент Арбитр дополняет свой граф
выполнения request_1.

Когда при дальнейшем выполнении request_1 нужно захватить объект Y
информация об этом снова передается Арбитру:

После захвата узла X Арбитр будет знать Z->Y->X

Такая же ситуация происходит с конкурентным requeest_2, он поочередно
захватывает К и M.

pic

Теперь запрос request_1 хочет захватить объект L, а конкурентный запрос
request_2 хочет захватить объект Y, захваченный другим запросом, таким
образом возникает угроза взаимоблокировки.

Но Арбитр при получении запроса на разрешение видит две цепочки:

request_1: Z->Y->X->(L)
request_2: K->L->M->(Y)

По этим двум цепочкам он строит граф выполняемых в данный момент запросов
и находит в нем цикл (подробнее могу расписать алгоритм нахождения цикла)
и выполняет отсечение ребер, чтобы устранить цикл. Среди вариантов
отсечения выбирается оптимальный по каким-то критериям и отсчеченные
варианты отменяются или откатываются


Каждому запросу (request) назначается арбитр (Arbiter) который управляет
запросом. Когда запрос по мере прохождения захватывает (блокирует) один
объект за другим, арбитру отправляется об этом уведомление.


какое должно быть правило для выбора кого отменять натуальный
идентификатор арбитра
...
#+results:
[[file:./arch-task-03.png]]

#+BEGIN_SRC plantuml :file ./arch-task-01.png
  participant A
  participant B
  'participant C
  participant D
...
#+results:
[[file:./arch-task-01.png]]


Сначала рассмотрим верхнюю часть - happy case.


Снаружи приходит запрос rq-0 на узел А.

Узел А вызывает узел B, передавае ему пару (rq-0, A). Так узел Б узнает от кого пришел вызов и куда вернуться, а также id запроса.


Вызывающий узел А замораживается на паре (rq-0, B), что означает, что он не будет работать с запросом rq-0 до тех пор, пока не получит сообщение с возвратом от B.


Если пока вызывающие узел А заблокирован придет сообщение с rq-0, но не с узла С (от которого мы ждем), то это значит мы задетектили рекурсию (не дедлок, об этом дальше)


Далее в happy case все очевидно, переходим ко второй части.


Механизм точно такой же - при вызове мы посылаем пару (ид.запроса, вызывающий узел) и блокируемся пока не получим возврат (ид.запроса, вызываемый узел)


Но, здесь у нас уже два запроса, rq-1 и rq-2. Там где синий крестик узел B получил неожиданное сообщение, но это не дедлок, потому что такой запрос можно притормозить, пока узел B ожидает на (rq-1, C) или до его таймаута, а потом сразу исполнить.


Блокировка же будет в том случае, если до того как узел С получил сообщение, он заблокировал другой узел (на диаграме - А)
...
Краткая, неполная, и в основном неверная схема работы Insolar,
...
очередной Пульс не придет - то вся сеть остановится. Также новый Пульс
содержит энтропию, в соответствии с которой в новом пульсе будут
переназначены Ноды на выполнение Ролей - это нужно для сменяемости нод.
Подробнее: [TODO:gmm] - Работа пульсаров.
...
исполняется запрос от пользователя. Мы будем рассматривать (простой)
