...
#+BEGIN_SRC go
  ...
  func ParseFile(fileName string, machineType insolar.MachineType) (*ParsedFile, error) {
      ...
      fmt.Printf("\n:: resource filename: %s", res.filename)
      ...
      res.code = sourceCode
      //fmt.Printf("\n:: resource code: %s", res.code)

      res.fileSet = token.NewFileSet()
      node, err := parser.ParseFile(res.fileSet, res.filename, res.code, parser.ParseComments)
      if err != nil {
  ...
  func (pf *ParsedFile) parseMethod(fn *ast.FuncDecl) error {
      // I want analise only method functions
      for _, fld := range fn.Recv.List {
          ...
          } else { // There is not method function - skip it
              return nil
          }
      }
