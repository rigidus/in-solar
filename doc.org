  ...
  func (pf *ParsedFile) parseMethod(fn *ast.FuncDecl) error {
      ...
      for _, fld := range fn.Recv.List {
          if 1 == len(fld.Names) { // There is method function
              ...
              // I want to analyse only methods, who receive context
              if isMethodTakesCtx(pars) {
                  // Debug prints
                  fmt.Printf("\n\nfn: %s", fn.Name.Name) // Function name
                  fmt.Printf("\nrecv: %s | %s", recv.Name, recv.Type) // Receiver
                  for _, par := range pars { // Parameters
                  ...
                  for _, smth := range fn.Body.List {  // ∀ fn.Body.List ← (or RetStmt (Inspect ...))
                      ...
                      if ok {
                          ...
                          collectRets(retStmt, pf.code, "Top")
                      } else {
                          ast.Inspect(smth, func(in ast.Node) bool {
                              ...
                              if ok {
                                  ...
                                  collectRets(retStmt, pf.code, "Deep")
                              } else {
