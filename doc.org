...
У нас есть SlotMachine, когда я, как игрок делаю ctx.Publish я могу
запаблишить SharedObject под ключом ("слот SM"+"номер матча"). Когда
второй игрок придет с тем же ключом - можно ctx.Unpublish этот
SharedObject но он будет разделяться между парой SM и его можно
использовать для взаимодействия внутри пары.Таким образом, нам нужна
только SM игрока, в которой следующие состояния:

- Find: создаем и регистрируем объект и ждем пока в нем не обновиться
  поле второго игрока, либо если объект уже зарегистрирован -
  дерегистрируем его, но сохраняем себе в SM и переходим в Game
- Game - если не наш ход ждем, если наш - бьем по мячу, иногда фейлимся,
  когда фэйл - переходим в GameOver
- GameOver - освобождаем объект и переходим в Find
...
workbook: unit-2, exer 1 2 3 page-11
