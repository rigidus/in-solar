...
Все SM знают ключ SharedObject-a, в котором происходит
формирование пар, потому что этот ключ в них зашит. Например это ключ
"make-pair".
...
переходит в состяние WaitForSecond и ждет на нем, когда поле SecondPlayer
в Shared-e изменится (если перепрыгнуть через следующий абзац, то можно
увидеть что будет дальше)

Если Shared существует, то SM записывает свой UID в поле SecondPlayer и
переходит в состояние WaitForGame, в котором ожидает, когда появится
