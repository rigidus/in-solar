  ...
  func ParseFile(fileName string, machineType insolar.MachineType) (*ParsedFile, error) {
      ...
      ast.Inspect(node, func(n ast.Node) bool {
          ...
          if ok {
              ...
              // Get fn contains
              fncont := fn.Body.List
              fmt.Printf("\nfncont (%d): %s", len(fncont), fncont)
              // Find all Return Statements
              for _, smth := range fncont {  // ∀ fn.Body.List ← (or RetStmt Inspect)
                  ...
                  if ok {
                      ...
                      ast.Inspect(smth, func(in ast.Node) bool {
                          ...
                          ret, ok := in.(*ast.ReturnStmt) // ←
                          if ok {
                              fmt.Printf("\nReturnStmt_2: %s | %s", reflect.TypeOf(ret), ret)
                          } else {
