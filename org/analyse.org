  ...
  func analyse(filename string) string {
      ...
      for 0 < len(unvisited) {
          ...
          if "NIL" == state.Migr {
              ...
              curr_migr = state.Migr
              uml += fmt.Sprintf("\n:curr_migr: set to `%s'", state.Migr)
          }

          uml += fmt.Sprintf("\n:curr_migr:[%s]", curr_migr)

          if curr_migr != "" {
              uml += fmt.Sprintf("\n%s -[#blue]-> %s", state.Name, curr_migr)
              if !isContainInSet(visited, curr_migr) {
                  unvisited = append(unvisited, curr_migr)
              }
          }
