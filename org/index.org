...
хранения ~DLT~, распределенный реестр.

Распределенный компьютер состоит из ~Node~, которые обмениваются
~Request~-ами.

~Request~ - это сетевой запрос от одного элемента системы - другому.

~Node~  - это компьютер в сети, который выполняет для сети какую-то
работу в соответствии со своей ~Role~.

~Role~ - это то какую работу ~Node~ делает делает для сети. Подробнее в
разделе [[*Node Roles][Node Roles]].
...
~Node~, которые исполняют свои ~Role~, остается постоянным.

Пульс задают ~Pulsar~-ноды, т.е. быть пульсаром - это одна из ~Ролей~.
[TODO:gmm] - Уточнить так ли это

Мы заинтереснованы в корректной работе пульсаров, потому что если
очередной пульс не придет - то вся сеть остановится.
[TODO:gmm] - Уточнить так ли это

Также новый пульс содержит энтропию, в соответствии с которой в новом
пульсе будут переназначены роли для нод - таким образом обеспечивается
сменяемость нод.  Подробнее: [TODO:gmm] - сделать раздел "Работа
пульсаров".

[TODO:gmm] - Зачем нужно менять роли?

Результатом их работы становится [TODO:gmm] - "Сообщение о пульсе",
которое распространяется по сети [TODO:gmm] - Как и кем?
...
- опеределить ~UserAddr~ - адрес контракта кошелька пользователя
- создать ~ctx~ - контекст исполнения запроса, структуру данных, которая
  будет сопровождать запрос
- записать ~Request~ в ~DLT~ [TODO:gmm] - Подробнее рассмотреть
- отправить подтверждение о том что ~Request~ принят тому, кто его
  отправил.
...
~Consume()~ вызывает кошелек принимающего, который:
